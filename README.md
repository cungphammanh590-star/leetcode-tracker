...
- **关窗（X）**：隐藏仪表盘，App 在后台保持桥接，扩展继续在线
- **退出（Cmd+Q 或程序坞右键退出）**：停止桥接，扩展会离线

## 常见问题

**App 在程序坞跳几下就退出，扩展显示「离线」**

1. 若从浏览器下载 ZIP，先在终端对 App 解除隔离：
   ```bash
   xattr -cr "/path/to/LeetCode Tracker.app"
   ```
2. 查看崩溃日志：`~/Library/Logs/leetcode-tracker-app.log`
3. 临时绕过：终端运行 `leetcode-tracker serve`，浏览器打开 http://127.0.0.1:8763/ ，扩展即可恢复在线
