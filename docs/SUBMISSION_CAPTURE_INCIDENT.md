...
```text
① POST /submit              → 只入库 + 成功通知（采集热路径唯一副作用）
② POST /api/coach/prepare   → /submit 成功之后另一次调用；读库 + 模板会话（无 LLM）
③ SSE /api/coach/stream     → 用户发消息后进入 LangGraph，按需调用 LLM
```

因此：

- `/submit` 不调用 LangGraph、Ollama、prepare 或 SSE；
- prepare / stream 失败不得改写采集成功态；
- 允许在 `/submit` **成功回包处理完毕后**单独调用 prepare，但不得把 prepare 并入 `/submit`，也不得恢复 Port 长连 / 双队列 / content 直连 bridge。
...
3. 不得在采集热路径（`/submit` 请求及其等待回包）中调用 Coach、LangGraph、Ollama 或其他 LLM；`/submit` 成功之后的独立 `prepare` 调用除外，且必须失败静默（`void prepareCoach`，勿 await 拖住 SW）。
4. 不得为采集增加第二条并行投递路径（content 直连、双队列等）；需要重试时应在 background 层对 `/submit` 单点实现。
5. 页面 Console 的 `emit submission` 只代表抓取成功；是否入库应以 `/submit` 响应或 SQLite 查询为准。
6. content → background 的 `sendMessage` **必须**带 response callback，以便在 `/submit` 完成前保持 SW 活跃。

## 附录：2026-07-20（FastAPI 迁移后复盘）

现象：控制台有 `emit submission` + `content relay`（例 `submission_id=736860735`，560 Accepted），仪表盘无记录。

对照：

- 迁移后 `/health.server == "fastapi"`，`curl POST /submit` 同 id **可写入** → 服务端写入正常。
- 库中原先无 `736860735` → 失败在扩展投递，而非 SQLite/图谱。
- 当时 content 使用无 callback 的 `sendMessage`，且 background 在 `sendResponse` 后 `await prepare`（LLM），易导致 SW 生命周期与采集交错。

修复（最小）：

1. 桥接改为 FastAPI + uvicorn（本 change `bridge-fastapi`）。
2. content `sendMessage` 增加 response callback。
3. prepare 改为 `void prepareCoach(...)`，不再 await。
